...
public class PlayerPrefsX
{
	...
	enum ArrayType {Float, Int32, Bool, String, Vector2, Vector3, Vector3i, Quaternion, Color}
	...
	public static bool SetVector3iArray (String key, Vector3i[] vector3iArray)
	{
		return SetValue (key, vector3iArray, ArrayType.Vector3i, 3, ConvertFromVector3i);
	}
	...
	private static void ConvertFromVector3i (Vector3i[] array, byte[] bytes, int i)
	{
		ConvertInt32ToBytes (array[i].x, bytes);
		ConvertInt32ToBytes (array[i].y, bytes);
		ConvertInt32ToBytes (array[i].z, bytes);
	}
	...
	public static Vector3i[] GetVector3iArray (String key)
	{
		var vector3iList = new List<Vector3i>();
		GetValue (key, vector3iList, ArrayType.Vector3i, 3, ConvertToVector3i);
		return vector3iList.ToArray();
	}

	public static Vector3i[] GetVector3iArray (String key, Vector3i defaultValue, int defaultSize)
	{
		if(PlayerPrefs.HasKey(key))
		{
			return GetVector3iArray(key);
		}
		var vector3iArray = new Vector3i[defaultSize];
		for (int i=0; i<defaultSize; i++)
		{
			vector3iArray[i] = defaultValue;
		}
	...
	public static void ConvertToVector3i (List<Vector3i> list, byte[] bytes)
	{
		list.Add (new Vector3i(ConvertBytesToInt32(bytes), ConvertBytesToInt32(bytes), ConvertBytesToInt32(bytes)));
	}

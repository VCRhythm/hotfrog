...
namespace HotFrog.Audio
{
	public class AudioPlayer : MonoBehaviour
	{
		...
		private void Awake()
		{
			...
			{
				channels[i] = transform.Find($"Channel{i + 1}").gameObject;
			}
		...
		private void SetMusicClipLoop(bool loop) => musicClipLoop = loop;

		private void SetMusicClipVolume(float volume) => musicClipVolume = volume;
		...
		private bool PlayMusicClip(int clipIndex)
		{
			...
			if (caCurrent.clip != null && caCurrent.isPlaying) // Music is playing on this channel
			{
				...
				{
					if (caOut.clip == AudioManager.Instance.musicClips[clipIndex]) // crossfading clip is the same one we need to fade in now
					{
						caCurrent = caOut;
						caOut = caIn;
						caIn = caCurrent;
		...
		private int GetNewChannelIndex() => channelIndex == 0 ? 1 : 0;
